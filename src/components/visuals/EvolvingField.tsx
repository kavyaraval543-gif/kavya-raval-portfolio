...
// Muted "black" base tone (subtle texture, not pure invisible-on-black)
// paired with the site's orange accent, which is the dominant dot color.
const PALETTE = {
  dark: { fg: "150, 140, 124", accent: "232, 99, 44" },
  ...
  useEffect(() => {
    ...
    function draw(time: number) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const radius = Math.max(0.4, p.r * 0.75);
        const d = radius * 2.9;

        ctx.globalAlpha = Math.min(1, p.alpha * (1 - p.accent));
        if (ctx.globalAlpha > 0.02) {
          ctx.drawImage(fgSprite, px - d / 2, py - d / 2, d, d);
        }

        if (p.accent > 0.02) {
          ctx.globalAlpha = Math.min(1, p.alpha * p.accent);
          ctx.drawImage(accentSprite, px - d / 2, py - d / 2, d, d);
        }
      }
