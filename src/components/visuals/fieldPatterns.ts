...
// reads as an abstract organic point cloud rather than a literal
// grid/ledger/punch-card illustration. The story is told through density
// and order, not iconography: early eras are sparse, loosely jittered, and
// irregular (hand-made); later eras are dense, tight, and precise
// (computed) — resolving into the final point-cloud wave.
...
function flow(i: number, n: number, p: FlowParams): FieldPoint {
  ...
  return {
    ...
    // Accent (orange) is the dominant dot color here, not a rare highlight —
    // most present points render orange; the rest carry the muted base tone.
    accent: present && rand2(i, 74) > 0.32 - amp * 0.3 ? 1 : 0,
...
export function waveField(i: number, n: number): FieldPoint {
  ...
  return {
    ...
    accent: rand2(i, 54) > 0.32 - amp * 0.3 ? 1 : 0,
    alpha: 0.5 + amp * 0.75,
  };
}
